...
- OpportunityScoreSensor: A debug sensor that shows the current opportunity score calculated by the agent for the current state.
- AnomalyIndexSensor: A debug sensor that shows the current anomaly index calculated by the agent based on sensor data.
- ActionMaskSensor: A debug sensor that shows the current action mask used by the agent to determine available actions based on context.
