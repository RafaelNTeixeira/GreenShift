...
- TaskStreakSensor: A sensor that tracks the user's current streak of days with completed tasks.
- WeeklyStreakSensor: A sensor that tracks the user's current streak of weeks with achieved weekly challenges.
...
class TaskStreakSensor(GreenShiftAISensor):
    """Sensor showing the current consecutive-day task-completion streak."""

    def __init__(self, agent):
        self._agent = agent
        self._attr_translation_key = "task_streak"
        self._attr_unique_id = f"{DOMAIN}_task_streak"
        self._attr_icon = "mdi:fire"
        self._attr_native_unit_of_measurement = "days"

    @property
    def state(self):
        """Return the current task streak length."""
        return self._agent.task_streak

    @property
    def extra_state_attributes(self):
        """Return last credited date."""
        last = self._agent.task_streak_last_date
        return {
            "last_credited_date": last.isoformat() if last else None,
        }


class WeeklyStreakSensor(GreenShiftAISensor):
    """Sensor showing the current consecutive-week challenge streak."""

    def __init__(self, agent):
        self._agent = agent
        self._attr_translation_key = "weekly_streak"
        self._attr_unique_id = f"{DOMAIN}_weekly_streak"
        self._attr_icon = "mdi:trophy"
        self._attr_native_unit_of_measurement = "weeks"

    @property
    def state(self):
        """Return the current weekly-challenge streak length."""
        return self._agent.weekly_streak

    @property
    def extra_state_attributes(self):
        """Return last credited week."""
        return {
            "last_credited_week": self._agent.weekly_streak_last_week,
        }
